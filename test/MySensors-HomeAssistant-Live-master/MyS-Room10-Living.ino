// Room sensor module
/* Sensors: 
 - 4 relays ; command 2 lights, webcam, electrical device
 - IR receiver; use a RC IR to command lights
 - IR send ; used for LG TV command
 - water leak
 - Motion (PIR)
 - Gas (MQ-2)
 - Temperature (DS18B20)
...
#define CHILD_ID_REL1 1 // Relay1
#define CHILD_ID_REL2 2 // Relay2
#define CHILD_ID_REL3 3 // Relay3 - used for WebCam
...
#define CHILD_ID_Tv 9 // Tv ON/OFF
...
#define REL1_PIN 8  // Relay1
#define REL2_PIN 7  // Relay2
#define REL3_PIN 6  // Relay3 used for WebCam
...
#define IRs_PIN 3 // IR send PIN for sending IR commands to Tv
...
MyMessage msgTv(CHILD_ID_Tv, V_LIGHT);
...
bool StateAlmWarn=0, StateAlmSnd=0, StateAlmActivate=0, StateTv=0;
...
void ServerUpdate() {
  ...
  send(msgPir.set(StatePIR ? "ON" : "OFF")); // needed in case of missing previous message
  ...
  if (Temp == Temp) send(msgTemp.set(Temp, 1));
  Hum = dht.getHumidity();
  if (Hum == Hum) send(msgHum.set(Hum, 1));
...
void IRr(){
  // read the IR remote for the manual light control
  unsigned long IC1=0;
  if (irrecv.decode(&results)) {
    IC1 = results.value;
    switch (IC1) {
      case 16724175: IrAct=1; break; // Corresponding button on the remote: 1 - Relay1
      case 16718055: IrAct=2; break; // 2 - Relay2
      //case 16743045: IrAct=3; break; // 3 - disabled for WebCam
      case 16716015: IrAct=4; break; // 4 - Relay4
      case 16769055: IrAct=10; break; // - Switch off all lights in the room
      case 16754775: IrAct=20; break; // + Switch on all lights in the room
      case 16748655: IrAct=31; break; // EQ - alm ON - Activate the Alarm for this room
      case 16761405: IrAct=30; break; // >| - alm OFF - Deactivate the alarm for this room
  ...
  switch(IrAct) {
      ...
      case 10: {
        ...
        //StateREL3=0; disabled for WebCam
        //digitalWrite(REL3_PIN, LOW);
      ...
      case 20: {
        ...
        digitalWrite(REL2_PIN, HIGH);
        send(msgREL2.set("ON"));
        //StateREL3=1; disabled for WebCam
        //digitalWrite(REL3_PIN, HIGH);
        //send(msgREL3.set("ON"));
        StateREL4=1;
        digitalWrite(REL4_PIN, HIGH);
 ...
 */
 ...
 /*IR LG
 * 551489775   20DF10EF   On/Off
 * 551520375   20DF8877   1
 * 551504055   20DF48B7   2
 * 551536695   20DFC837   3
 * 551495895   20DF28D7   4
 * 551528535   20DFA857   5
 * 551512215   20DF6897   6
 * 551544855   20DFE817   7
 * 551491815   20DF18E7   8
 * 551524455   20DF9867   9
 * 551487735   20DF08F7   0
 * 
 * 551502015   20DF40BF   V+
 * 551534655   20DFC03F   V-
 * 
 * 551485695   20DF00FF   P+
 * 551518335   20DF807F   P-
 */ 
